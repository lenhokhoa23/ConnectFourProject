...
def winning_move(board, piece):
    # Horizontal, vertical, and diagonal checks remain unchanged
...
def evaluate_window(window, piece):
    ...
    if window.count(piece) == 4:
        ...
        score += 50  # Increased score for three with an empty
    elif window.count(piece) == 2 and window.count(EMPTY) == 2:
        score += 10

    if window.count(opponent_piece) == 3 and window.count(EMPTY) == 1:
        score -= 100  # Greater penalty for the opponent's three
...
def score_board(board):
    ...
    score += center_count * 3  # Center control
...
def connect4_game():
    ...
    while not game_over:
        if turn == PLAYER_1:  # Player 1's turn
            ...
            col, _ = minimax(board, 5, -np.inf, np.inf, True)  # AI plays optimally.
            if is_valid_location(board, col):
