...
def find_best_move(board, depth):
    ...
    for col in range(7):
        if board.drop_piece(col, 1):  # AI là người chơi 1
...
def play_game(replay_memory):
    ...
    while not board.is_full():
        ...
        if current_player == 1:
            column = find_best_move(board, 5)  # Sử dụng độ sâu hợp lý
            if column != -1:
            ...
            column = int(input("Người chơi 2 (Người), chọn một cột (0-6): "))
            if column < 0 or column >= 7 or not board.drop_piece(column, current_player):
        ...
        current_player = 3 - current_player  # Chuyển đổi người chơi
